...
trainer_stats_name="resource_usage_step"
...
class ResourceStats(base.TrainerStats):
    ...
    def __init__(self, device: torch.device, conf: config.Config):
        ...
        # create the CPU process for CPU measurements
        self.process = psutil.Process()

        # data for the resource utilization
        self.step_resource_data = stats.ResourceStatsData(conf, self.handle, self.process, self.device)        
    ...
    def start_step(self):
        ...
        self.step_resource_data.start()

        
    def stop_step(self):
        torch.cuda.synchronize(self.device)
        self.step_resource_data.stop()
    ...
    def log_stats(self):
        self.step_resource_data.to_csv(exp="resource", dir="resource_data", step="step")
